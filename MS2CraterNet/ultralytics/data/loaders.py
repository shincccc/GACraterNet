...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            #dt_path = path.replace("coco_rgb", "coco_dem").replace(fold, "alldata").replace(name, name.replace("co", 'dt'))
            dt_path = './data/dem_clips'
            if os.path.exists(dt_path):
                im0_dept = cv2.imread(path)
            if path.endswith("bus.jpg"):
                im0_dept = cv2.imread(path)
            if im0 is None:
                raise FileNotFoundError(f"Image Not Found {path}")
            s = f"image {self.count}/{self.nf} {path}: "

        return [path], [im0],[im0_dept], self.cap, s
